...
| Command | Result |
|---|---|
| `CAM_ASSISTANT_SKIP_FRESH_CLONE=1 /bin/zsh scripts/verify.sh all` plus `/bin/zsh scripts/verify.sh smoke` | Repository portability and honest 48-gate-map checks plus all 287 tests pass; native app and CLI release build, two-build package reproducibility (`2a63ff157c9d3e3b19323b833fe0c3a93c604c1c0310b1e1d6341a8bfd7d062d`), package identity, 54-file zero-finding credential-signature scan, and offline smoke pass |
...
| `/bin/zsh scripts/verify.sh app` | All 22 app tests pass, including injected repository-semantic states, explicit retention, repository recovery, bounded backup/recovery, research proposal/block/acquire/cancel/recover/review/Keep/Discard/restart behavior, and a real coordinator/store cancellation race |
...
| `/bin/zsh scripts/verify.sh fresh-clone` | Temporary non-local clone of repository-semantic implementation checkpoint `4193820165d12d057f512bce17350bf02f4b6dd6` passes portability, the honest 48-gate map, all 260 tests, app/CLI release builds, package reproducibility/identity with `dirty=false`, a 53-file zero-finding credential-signature scan, and offline smoke from a clean source tree |
...
| `Tests/ReleaseProofTests/verify-release-privacy-suite-tests.sh` | Package identity, scanner clean/failure/redaction contracts pass; the latest aggregate release package and saved evidence scan passes with 54 files and zero credential-signature findings; atomic JSON receipt saved |
| `/bin/zsh scripts/verify.sh goal-map` | Pinned-goal validator covers all 48 Proof-of-Done bullets and reports the honest current verdict: 16 passed, 26 partial, 6 missing, overall incomplete |
...
This report does not claim that CAM mining, app-owned live CAM probing,
