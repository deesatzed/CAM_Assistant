...
| Command | Result |
|---|---|
| `CAM_ASSISTANT_SKIP_FRESH_CLONE=1 /bin/zsh scripts/verify.sh all` plus direct current debug-binary `--smoke-offline` | Repository portability and honest 48-gate-map checks plus all 312 tests pass; native app and CLI release build, two-build package reproducibility (`9ae5537913528eae2e4e9a3f9bc1744ad6344f0ad32a3db18710bc1e2d8d1542`), package identity, 56-file zero-finding credential-signature scan, and offline smoke pass |
...
| `/bin/zsh scripts/verify.sh cam` | All 30 CAM tests pass: 25 bounded runtime/probe/CLI tests plus 5 atomic restart-state tests covering validated pin/receipt restart, stale-receipt replacement, corrupt/unsupported state, mismatched binding, and malformed verified-receipt refusal |
...
| `/bin/zsh scripts/verify.sh fresh-clone` | Temporary non-local clone of native CAM runtime checkpoint `ef2a7bf8e03d2de4b58317bb83885483fa42396f` passes portability, the honest 48-gate map, all 307 tests, app/CLI release builds, package reproducibility (`eecd0db30b046519a1ba12bf9643d4186cac611ef5aa33f3671462b3932db596`), package identity with `dirty=false`, a 55-file zero-finding credential-signature scan, and offline smoke from a clean source tree |
...
| CAM/Codex coordination | Fixture-pinned CAM contract/schema, unavailable/incompatible handling, proposal-only adapter; one bounded local loop with OS-file-lock ownership, content-addressed evidence, reducer/replay/snapshot/migration/handoff proof; native/CLI runtime selection that derives and drift-checks the installed runtime and WAL-family corpus; a bounded native disposable statistics verifier; and atomic restart state that revalidates a saved pin/receipt, labels it historical, and requires a current-session re-pin before another probe | Partial: the safe native verifier deliberately does not launch CAM; packaged restart proof, durable live-run recovery, closed live CAM tool execution, exact-approved mining, retries/idempotency, verified mining postconditions, and complete coordination controls remain |
...
5. Preserve the local CAM/Codex historical restart-state boundary, prove it in
   a packaged restart journey, then add a closed safe tool/retry executor,
   verified postconditions, durable live-run recovery, and complete native
