...
| Command | Result |
|---|---|
| `CAM_ASSISTANT_SKIP_FRESH_CLONE=1 /bin/zsh scripts/verify.sh all` | Repository portability and 48-gate-map checks plus all 260 tests pass; native app and CLI release build, two-build package reproducibility, package identity, 53-file zero-finding credential-signature scan, and offline smoke pass |
...
| `/bin/zsh scripts/verify.sh app` | All 16 app tests pass, including injected repository-semantic accepted/abstained/stale/unavailable/insufficient/cancelled states, explicit retention controls, live-lease recovery, cancellation reload, and non-blocking bounded backup/recovery controls |
